...
class Major(db.Model):
    ...
    students_in_major : sqlo.WriteOnlyMapped["Student"] = sqlo.relationship(
        secondary = student_major_table,
        primaryjoin = (student_major_table.c.major_id == id),
        back_populates= "major_of_student")
...
class Student(UserMixin, db.Model):
    ...
    password_hash : sqlo.Mapped[Optional[str]] = sqlo.mapped_column(sqla.String(256), index = True, unique = True)
    ...
    #relationships
    major_of_student : sqlo.WriteOnlyMapped["Major"] = sqlo.relationship(
        secondary = student_major_table,
        primaryjoin = (student_major_table.c.student_id == id),
    ...
    #methods
    def __repr__(self):
        return "<Student {} - {} - {} {} - {} - {};>".format(self.id, self.username, self.firstname, self.lastname, self.email, self.address)
    ...
    def get_majors(self):
        query = self.major_of_student.select()
        return db.session.scalars(query).all()
    
    def is_enrolled(self, new_course):
        result = db.session.scalars(self.enrollments.select().where(Enrolled.course_id == new_course.id)).first()
        return result is not None
    
    def enroll(self, new_course):
        if not self.is_enrolled(new_course):
            new_enrollment = Enrolled(course_enrolled = new_course, student_enrolled = self)
            db.session.add(new_enrollment)
            db.session.commit()   
    
    def unenroll(self, old_course):
        if self.is_enrolled(old_course):
            cur_enrollment = db.session.scalars(self.enrollments.select().where(Enrolled.course_id == old_course.id)).first()
    ...
    def get_enrollment_date(self, the_course):
        if self.is_enrolled(the_course):
            cur_enrollment = db.session.scalars(self.enrollments.select().where(Enrolled.course_id == the_course.id)).first()
            return cur_enrollment.enroll_date

    
class Dnrolled(db.Model):
